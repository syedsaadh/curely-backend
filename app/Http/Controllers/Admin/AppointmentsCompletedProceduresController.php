...
use App\Models\AppointmentCompletedProcedures;
...
class AppointmentsCompletedProceduresController extends Controller
{
    public function store(Request $request)
    {
        ...
        foreach ($toDelete as $item) {
            try {
                AppointmentCompletedProcedures::find($item['id'])->delete();
            } catch (QueryException $e) {
            }
        }
        foreach ($toUpdate as $item) {
            try {
                $it = AppointmentCompletedProcedures::find($item['id']);
                if ($it) {
        ...
        foreach ($toInsert as $item) {
            try {
                $it = new AppointmentCompletedProcedures();
